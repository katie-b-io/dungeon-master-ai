...
class MonsterCollection(metaclass=MonsterCollectionMeta):
    ...
    @classmethod
    def get_monster(cls, monster_cls: str, state: State, unique_id: str = None, unique_name: str = None) -> Monster:
        """Return an instance of a monster of specified type"""
        try:
            return cls._monster_factory(monster_cls=monster_cls, state=state, unique_id=unique_id, unique_name=unique_name)
        except ValueError as e:
            logger.error(e)

    @classmethod
    def get_monster_npc(cls, npc_data: dict, state: State) -> NPC:
        """Return an instance of a monster of specified NPC."""
        try:
            monster = cls._monster_factory(npc_data=npc_data, state=state, unique_id=npc_data["id"], unique_name=npc_data["name"])
    ...
    def _monster_factory(cls,
        ...
        try:
            ...
            return monster(cls.monster_data[monster_cls], state, npc_data, unique_id, unique_name)
